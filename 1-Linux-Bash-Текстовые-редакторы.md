...
5. Активно начать пользоваться ботом: https://t.me/gigachat_bot который ответит на любые вопросы (например спросите у него `Расскажи как ребенку, зачем DevOps нужен Kubernetes`)
6. Зарегистрироваться на сайтах:
   - https://github.com - Сайт, на котором множество разработчиков размещают бесплатно код своих программ или, например, размещен этот курс
   - https://habr.com/ru/flows/admin - Русскоязычный сайт с кучей статей по DevOps, разработке и автоматизации
...
1. Как сделать так, чтобы при выполнении команды `sudo` у нас не спрашивался каждый раз пароль?
2. Перейди в `/tmp` директорию
3. Вернитесь обратно в директорию, в которой были первоначально
4. Перейди в домашнюю директорию пользователя
5. Перейди в вышестоящую директорию
6. Перейди в корень (что такое корень?)
7. Выполни команду `cd ~`, куда вы попали?
8. Выведи на экран полный путь к директории, где ты сейчас находишься
9. Создай два файла с именами `config` и `binary`
10. Создай директорию `app`
11. Перемести файлы `config` и `binary` в директорию `app`
12. Создай в директории `app` подпапки `temp/info/users` одной командой
13. Вернись на директорию выше и удали директорию `app`
14. Обьяснить, какие данные будут выведены на экран в команде `top`
15. Какой командой можно посмотреть использование оперативной памяти?
16. Посмотреть, сколько места занято на диске `/` и насколько он нагружен
17. Выполните команду `uptime`, что такое load average?
18. Какие основные директории есть в `/` «корне»? 
...
2. Вывести все файлы из папки `/etc`, в которых есть имя вашего пользователя
