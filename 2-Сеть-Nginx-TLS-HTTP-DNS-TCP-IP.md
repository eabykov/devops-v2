...
3. Сетевой уровень (Internet Layer) этот уровень отвечает за маршрутизацию пакетов (частичка инофрмации которая передается по сети, в нем есть информация, которую нужно передать, и адрес, куда ее доставить). Ключевой протокол:
- IP (Internet Protocol) – основной протокол, обеспечивающий адресацию и доставку пакетов от одного сервера к другому
