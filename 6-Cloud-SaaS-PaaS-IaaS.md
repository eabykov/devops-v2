...
1. **Что такое модель разделяемой ответственности?**
   **Ответ:** Договорённость о том, кто за что отвечает. Провайдер отвечает за безопасность **самого** облака: дата-центры, железо, гипервизор, сеть. Клиент — за безопасность **в** облаке: настройку доступов, шифрование данных, обновления операционной системы на своих машинах, правила групп безопасности. Большинство утечек происходит по вине клиента, а не провайдера: типичный случай — бакет, открытый на публичное чтение

2. **Чем зона доступности отличается от региона и как выбирать регион?**
   **Ответ:** Регион — географическая локация, зона — отдельный дата-центр внутри неё. Регион выбирают по трём критериям: близость к пользователям (задержка), требования законодательства к месту хранения данных и цена, которая между регионами различается. Между зонами одного региона задержка порядка миллисекунды, между регионами — десятки миллисекунд, и это уже влияет на архитектуру

3. **Приложение должно пережить отказ целого региона. Что это меняет?**
   **Ответ:** Многое и дорого. Нужны копии в двух регионах, репликация данных между ними (а это задержка и вопрос согласованности), балансировка на уровне DNS, регулярные учения по переключению. Прежде чем строить такое, отвечают на вопрос, сколько стоит час простоя: для большинства систем достаточно нескольких зон в одном регионе

4. **Что такое сервисный аккаунт в облаке и чем он лучше ключей доступа?**
   **Ответ:** Учётная запись для программы, а не человека. Виртуальная машина или под получает временные автоматически обновляемые полномочия просто по факту того, чем она является. Постоянные ключи доступа при этом не нужны, а именно они утекают в репозитории чаще всего. Правило: долгоживущий ключ — это последнее средство

5. **Что такое Infrastructure as Code и почему в облаке без него плохо?**
   **Ответ:** Описание инфраструктуры текстовыми файлами в репозитории. Без него окружения расползаются (в тестовом одно, в боевом другое, и никто не помнит почему), забытые ресурсы тратят деньги, а воспроизвести конфигурацию после аварии невозможно. Подробно — тема 7

6. **Как в облаке устроено резервное копирование и что с ним чаще всего не так?**
   **Ответ:** Снимки дисков, автоматические копии управляемых баз, версионирование объектных хранилищ. Главная проблема не в создании копий, а в том, что их не проверяют: восстановление впервые пробуют во время аварии и обнаруживают, что копия пуста или неполна. Правило: копия существует, только если из неё успешно восстановились — в теме 8 это один из пунктов чек-листа

7. **Почему счёт за облако вырос вдвое? Как искать причину?**
   **Ответ:** По разбивке затрат в консоли биллинга: по сервисам, по тегам, по проектам. Частые виновники: забытые машины и диски от удалённых машин, неиспользуемые статические адреса, исходящий трафик (он почти всегда платный, а входящий обычно нет), логи и метрики с большим сроком хранения, снимки дисков. Именно поэтому теги на ресурсах обязательны: без них невозможно понять, чьё это

8. **Чем управляемый Kubernetes отличается от кластера, поднятого вручную?**
   **Ответ:** Провайдер берёт на себя управляющие ноды: их доступность, обновление, резервное копирование etcd. Ты платишь и отвечаешь только за рабочие ноды и свои приложения. Самостоятельная установка оправдана редко: когда нужны нестандартные настройки API-сервера или кластер работает вне облака

9. **Что такое прерываемые (spot) машины и как на них строить работу?**
   **Ответ:** Дешёвые машины из свободных мощностей, которые провайдер может забрать, предупредив за короткое время. Подходят для того, что переживает потерю узла: пакетная обработка, тестовые окружения, часть нод Kubernetes-кластера. Правила: не держать на них состояние, корректно обрабатывать сигнал завершения и всегда иметь запас обычных нод

10. **Как правильно организовать доступы в облаке?**
    **Ответ:** Права выдавать группам и ролям, а не людям поимённо; минимально необходимый набор; отдельные проекты или каталоги под окружения, чтобы тестовое не имело доступа к боевому; обязательная двухфакторная аутентификация; корневой аккаунт не использовать в повседневной работе; включённый журнал аудита. Разбор инцидента начинается с вопроса «кто и что мог сделать», и ответ на него должен быть в журнале

11. **Приложение в облаке стало медленно работать. С чего начать?**
    **Ответ:** Понять, где именно медленно: у пользователя, на балансировщике, в приложении или в базе — это задача мониторинга из темы 8. Специфично для облака проверяют: не упёрлись ли в квоты и лимиты сервиса, не исчерпан ли запас производительности диска (у дешёвых дисков он накопительный и заканчивается), не работает ли машина на сниженной доле процессора, не ходит ли трафик между зонами там, где не должен

12. **Что важно учесть при переезде из одного облака в другое?**
    **Ответ:** Больше всего проблем создают данные: их объём, время переноса и плата за исходящий трафик. Дальше — уникальные сервисы провайдера, которым нет прямого аналога, различия в сетевой модели и правах доступа. Переезд упрощает опора на переносимые технологии: Kubernetes, PostgreSQL, S3-совместимое хранилище, инфраструктура как код
