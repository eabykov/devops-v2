...
8. **Чем `count` отличается от `for_each`?**
   **Ответ:** `count` создаёт ресурсы по числовому индексу, `for_each` — по ключам из множества или словаря. Разница проявляется при удалении: убрав второй элемент из списка при `count`, ты сдвинешь индексы, и Terraform пересоздаст все последующие ресурсы. При `for_each` удалится ровно один — остальные привязаны к своим ключам. Практическое правило: `for_each` почти всегда предпочтительнее

9. **Зачем нужен блок `lifecycle`?**
   **Ответ:** `prevent_destroy` запрещает удаление ресурса — ставят на боевые базы и хранилища. `ignore_changes` велит не реагировать на изменения поля, которое меняет кто-то другой (например, число нод, управляемое автоскейлером), иначе Terraform будет откатывать его при каждом запуске. `create_before_destroy` создаёт замену до удаления старого, чтобы не было простоя

10. **Ресурс изменили руками в консоли. Что сделает Terraform?**
    **Ответ:** При следующем `plan` он сравнит реальное состояние с описанным и предложит вернуть как в коде. Это и есть исправление дрейфа. Если изменение было нужным, его переносят в код, а не оставляют в консоли — иначе следующий `apply` его снесёт, причём в самый неподходящий момент

11. **Как заставить Terraform пересоздать один конкретный ресурс?**
    **Ответ:** `terraform apply -replace=адрес_ресурса`. Раньше для этого использовали `terraform taint`, сейчас он считается устаревшим. Нужно, когда ресурс «испортился» способом, невидимым для Terraform: например, внутри машины что-то сломали руками

12. **Как переименовать ресурс, не пересоздавая его?**
    **Ответ:** Блоком `moved` в коде — он объясняет Terraform, что ресурс не удалён и создан заново, а просто сменил адрес. Старый способ — `terraform state mv`. Без этого простое переименование в коде приведёт к удалению и созданию заново, что для базы данных означает потерю данных

13. **Зачем нужна блокировка state и что будет без неё?**
    **Ответ:** Она не даёт двум `apply` работать одновременно. Без блокировки параллельные запуски перезапишут состояние друг друга, и оно перестанет соответствовать реальности — часть ресурсов окажется «потеряна». Реализуется средствами хранилища; в S3-совместимых бакетах для этого используют отдельный механизм блокировок

14. **Почему `provisioner` считается плохой практикой?**
    **Ответ:** Он выполняет команды на созданной машине, но Terraform не отслеживает результат: повторный запуск ничего не проверит, а при ошибке ресурс останется в подвешенном состоянии. Настройка машин — задача Ansible или облачного механизма первоначальной инициализации (`user_data`, cloud-init). Официальная документация сама называет provisioner последним средством

15. **Как в Terraform разделить окружения?**
    **Ответ:** Основных подхода два. Отдельные каталоги с собственным state на каждое окружение и общим кодом в модулях — многословно, но нагляднее и безопаснее. Workspaces — один код и несколько состояний, компактно, но легко перепутать текущее окружение и применить изменения не туда. В командах чаще выбирают первый вариант

#### Ansible глубже

16. **Что такое inventory и каким он бывает?**
    **Ответ:** Список управляемых машин, сгруппированных по ролям. Статический — файл `hosts.ini` или YAML. Динамический — скрипт или плагин, который получает список из облака по API. В облаке нужен динамический: машины появляются и исчезают, а руками список никто не поддержит

17. **Как хранить секреты в Ansible?**
    **Ответ:** `ansible-vault` шифрует файл или отдельные значения, пароль передаётся при запуске. Такой файл можно держать в репозитории. Более зрелый вариант — забирать секреты из внешнего хранилища через плагин поиска в момент выполнения, тогда в репозитории не лежит даже зашифрованного значения

18. **Что делает режим `--check` и в чём его ограничение?**
    **Ответ:** «Сухой прогон»: Ansible показывает, что изменилось бы, ничего не меняя, — аналог `terraform plan`. Ограничение в том, что задачи с `command` и `shell` в этом режиме просто пропускаются, и результат получается неполным. Чем больше специализированных модулей вместо произвольных команд, тем полезнее `--check`

19. **Как обновлять серверы по очереди, а не все сразу?**
    **Ответ:** Параметром `serial`: `serial: 1` — по одному, `serial: 25%` — по четверти за раз. Вместе с `max_fail_percentage` это даёт постепенное обновление с остановкой при ошибках — тот же принцип, что у rolling update в Kubernetes, только для обычных машин

20. **Зачем нужен `gather_facts` и когда его отключают?**
    **Ответ:** В начале работы Ansible собирает сведения о машине: операционную систему, адреса, объём памяти — их потом используют в условиях и шаблонах. Сбор занимает время, поэтому на больших парках машин его отключают (`gather_facts: false`), если факты не нужны, либо кешируют

21. **Что такое `delegate_to` и `run_once`?**
    **Ответ:** `delegate_to` выполняет задачу на другой машине, чем текущая: например, вывести сервер из балансировщика — эта команда должна выполниться на балансировщике, а не на самом сервере. `run_once` выполняет задачу один раз, а не на каждом хосте группы — так делают миграции базы
