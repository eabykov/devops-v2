...
16. **Как сделать условие для булева значения и для строки?**
    **Ответ:** Для булева — {% raw %}`{{ if .Values.ingress.enabled }} ... {{ end }}`{% endraw %}, для строки — сравнение: {% raw %}`{{ if eq .Values.env "prod" }}`{% endraw %}. Подвох: шаблонизатор считает ложью не только `false`, но и пустую строку, ноль и пустой список. Поэтому для «значение вообще задано?» надёжнее {% raw %}`{{ if hasKey .Values "replicaCount" }}`{% endraw %}

17. **Что вернёт {% raw %}`{{ divf .Values.replicaCount .Values.zones | ceil }}`{% endraw %}?**
...
- Забыть {% raw %}`{{- ... }}`{% endraw %} с дефисом там, где нужно убрать лишний перенос строки — YAML разъедется
